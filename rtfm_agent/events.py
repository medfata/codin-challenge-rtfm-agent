"""Step 11: real-time event notifications over a per-tenant Redis Stream.
...
def normalize_last_id(raw: str | None) -> str:
...
def publish(r: Redis, t: TenantContext, type_: str, data: dict) -> str | None:
    ...
    (~100 entries), so short streams would never shrink. Payload sizes and
    publish rates (per ingestion/turn, not per token) make exact trim cost
    negligible and keep the memory bound honest.
    """
...
async def aresolve_start(client: AsyncRedis, t: TenantContext,
    ...
    predecessor (fetching N+1 to find it; "0" when the stream is shorter
    than the backlog). Else "$" = only entries newer than now. A malformed
    Last-Event-ID is ignored (falls back to "$") instead of erroring forever.
    """
    last_event_id = normalize_last_id(last_event_id)
    if last_event_id:
        return last_event_id
...
def iter_events(client: Redis, t: TenantContext, cursor: str):
    ...
    of raising. (The SSE endpoint uses aiter_events; this stays available
    for tests and CLI tools.)
    """
    key = stream_key(t)
    block_ms = max(EVENTS_HEARTBEAT_S * 1000, 1000)
    while True:
        try:
