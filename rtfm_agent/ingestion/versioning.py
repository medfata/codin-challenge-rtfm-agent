"""Document versioning: content-hash corpus tracking + staleness warnings.
...
from rtfm_agent.ingestion.documents import DOC_EXTENSIONS
...
def web_docs_root(t: TenantContext) -> Path:
    """This tenant's crawled-pages root, e.g. docs/web/<org>/."""
...
def prepare(r: Redis, t: TenantContext, docs: list[dict]) -> dict:
    ...
    if not settings.versioning.enabled:
        return empty
...
def scan_drift(r: Redis, t: TenantContext, docs_dir: str | Path,
    ...
    if not force:
        ...
        if cached is not None and now - cached[0] < settings.versioning.drift_scan_ttl_s:
            return cached[1]
    ...
    # Crawled pages live outside the primary docs dir but are part of the
    # same corpus (indexed as `web/<host>/<page>.md`). Scan them too, or
    # every approved page would permanently show up as "removed" drift.
    try:
...
def answer_staleness(r: Redis, t: TenantContext, docs_dir: str | None = None,
    ...
    if cached_version is not None:
        ...
        if msg:
            reasons.append(msg)
    if settings.versioning.drift_warnings and docs_dir:
        try:
...
def status_report(r: Redis, t: TenantContext, docs_dir: str | None = None) -> dict:
    ...
    report = {
        "tenant": t.id,
        "versioning_enabled": settings.versioning.enabled,
        "corpus": None,
        "drift": {},
        "up_to_date": True,
    }
