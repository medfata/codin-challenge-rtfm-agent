...
from rtfm_agent.common.events import INGEST_COMPLETED, INGEST_STARTED, publish
...
def collect_docs(docs_dir: str | Path, t: TenantContext) -> list[dict]:
    ...
    web_root = versioning.web_docs_root(t)
    if web_root.is_dir():
...
def create_redis_index(r: Redis, t: TenantContext, dim: int) -> bool:
    ...
    def needs_recreate() -> str | None:
        ...
        if find_value(info, "dim") != dim:
            return "dim"
        # Migration: source_file must be TAG (was TEXT pre-hybrid-search)
        attrs = None
...
def run_ingestion(r: Redis, t: TenantContext, docs_dir: str | Path = None,
    ...
    """
    docs_dir = str(docs_dir or settings.docs.dir)
    t0 = time.time()
    publish(r, t, INGEST_STARTED,
    ...
    # version it belongs to (no-op when versioning disabled).
    prep = versioning.prepare(r, t, docs)

    log("[4/4] Storing in Redis...")
    removed = delete_source_keys(r, t)
    created = create_redis_index(r, t, settings.embed.dim)
    chunks_with_emb = [
        {**c, "embedding": e, "doc_version": prep["version"]}
        for c, e in zip(chunks, embeddings)
    ]
    stored = store_in_redis(r, t, chunks_with_emb)
    version_summary = versioning.finalize(r, t, prep, chunks)
    ...
    summary = {
        ...
        "index": t.doc_index,
        "embedding_dim": settings.embed.dim,
        **version_summary,
        "duration_s": duration,
    }
    log(f"done in {duration}s")
    publish(r, t, INGEST_COMPLETED, summary)
    return summary
