...
(see memory.py). The strict-slug validation prevents key-space injection:
a tenant id can never contain ':' or wildcard characters.
"""
...
from rtfm_agent.config import TENANTS_OPEN, TENANT_ALLOWLIST
...
def normalize_tenant(raw: str | None) -> TenantContext | None:
    ...
    if not _TENANT_RE.match(tenant_id):
        return None
    if not TENANTS_OPEN and tenant_id not in TENANT_ALLOWLIST:
        return None
