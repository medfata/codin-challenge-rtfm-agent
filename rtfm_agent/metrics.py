...
from rtfm_agent.config import PRICE_PER_MTOK_INPUT, PRICE_PER_MTOK_OUTPUT
...
def record_request(r: Redis, t: TenantContext, prompt_tokens: int = 0,
    ...
    try:
        ...
        if prompt_tokens:
            ...
            cost = prompt_tokens / 1e6 * PRICE_PER_MTOK_INPUT
        ...
        if completion_tokens:
            ...
            cost = completion_tokens / 1e6 * PRICE_PER_MTOK_OUTPUT
            pipe.hincrbyfloat(t.metrics_key, "estimated_cost_usd_total", cost)
        pipe.execute()
    except Exception as exc:
        import logging
