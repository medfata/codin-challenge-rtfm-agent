"""Step 1: Chunk documentation text into ~500 token segments with paragraph overlap."""
...
def _apply_overlap_prev(chunk_str: str, prev_tail: str) -> str:
    ...
    Otherwise, prepend a sensible overlap portion (~50 chars / ~120 chars).
    """
    if not prev_tail:
        return chunk_str

    # Take last portion for overlap (up to overlap_chars)
...
def load_and_chunk_docs(docs, max_chunks_per_doc=None) -> list:
    ...
    Args:
        docs: List of dicts from step1_load_docs.load_asc_files()
