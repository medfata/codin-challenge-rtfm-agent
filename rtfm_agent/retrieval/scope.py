...
from rtfm_agent.common.redis_utils import normalize
...
def get_inventory(r: Redis, t: TenantContext) -> list[str]:
    ...
    try:
        ...
        data = normalize(res).get("results") or []
        files = []
        for row in data:
...
def match_files(hint: str, inventory: list[str],
                top_n: int | None = None) -> list[str]:
    """Fuzzy-match a hint against file paths via token overlap."""
    top_n = top_n or settings.retrieval.scope_top_n_files
