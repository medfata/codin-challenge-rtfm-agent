...
from rtfm_agent.common.events import MEMORY_TURN_STORED, publish
...
def is_healthy() -> bool:
    try:
        r = httpx.get(f"{settings.memory.server_url}/v1/health", timeout=5)
...
def search_memories(text: str, t: TenantContext, limit: int | None = None) -> list[dict]:
    ...
    """
    if not settings.memory.enabled or not text.strip():
        return []
    try:
        r = httpx.post(
            f"{settings.memory.server_url}/v1/long-term-memory/search",
            json={
                "text": text,
                "limit": limit or settings.memory.search_limit,
...
def save_turn(session_id: str, question: str, answer: str, t: TenantContext,
    ...
    response must be checked explicitly.
    """
    if not settings.memory.enabled:
        return
    url = f"{settings.memory.server_url}/v1/working-memory/{session_id}"
    try:
        messages: list[dict] = []
        ...
        if saved.status_code >= 400:
            ...
            publish(r, t, MEMORY_TURN_STORED, {"session_id": session_id})
    except Exception as exc:
