...
def check_lane_config() -> None:
    ...
    pinned = all(m in (settings.llm.economy_model, settings.llm.economy_fallback_model)
                 for _n, m in models)
    check("Economy lanes pin their models", pinned, f"{models}")
    check(
        "Economy fallback is NOT the fast-lane model",
        settings.llm.economy_fallback_model != settings.llm.fast_model,
        f"economy_fb={settings.llm.economy_fallback_model!r} fast={settings.llm.fast_model!r}",
    )
...
def check_economy_unconfigured_failopen() -> None:
    ...
    original_key = settings.llm.economy_api_key
    try:
        settings.llm.economy_api_key = ""
        ...
        settings.llm.economy_api_key = original_key
