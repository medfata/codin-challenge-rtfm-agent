...
def part_b_redis() -> None:
    try:
        r = Redis.from_url(settings.redis.url, decode_responses=False, socket_connect_timeout=3)
        r.ping()
    except Exception as exc:
        skip("Redis drills", f"no Redis at {settings.redis.url}: {exc}")
        return
    ...
    try:
        ...
        qvec = np.zeros(settings.embed.dim, dtype=np.float32).tobytes()
