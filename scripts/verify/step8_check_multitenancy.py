...
from rtfm_agent.common.tenancy import TenantContext, normalize_tenant, require_tenant
...
def check_tenancy_units() -> None:
    ...
    if settings.tenants.open or "acme" in settings.tenants.allowlist:
    ...
    if not settings.tenants.open:
        off_list = "off-list-step8-probe"
        while off_list in settings.tenants.allowlist:
            off_list += "x"
        try:
...
def _synthetic_vector() -> np.ndarray:
    """Fixed random float32 vector of settings.embed.dim floats (rng seed 42)."""
    return np.random.default_rng(42).standard_normal(settings.embed.dim).astype(np.float32)
...
def run_redis_drills(r: Redis) -> bool:
    ...
    try:
        ...
        skip("Redis isolation drills",
             f"unreachable at {settings.redis.url}: {str(exc)[:150]}")
        return False
    ...
    try:
        try:
            idx_a = ingest_mod.create_redis_index(r, a, settings.embed.dim)
            idx_b = ingest_mod.create_redis_index(r, b, settings.embed.dim)
